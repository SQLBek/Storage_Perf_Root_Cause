...
-- Used w. Permission 
-- Changelog:
--    * Reformatted w. Poor Man's T-SQL Formatter
--    * Added WHERE DB name predicate for demo filtering
-------------------------------------------------------------------*/
...
SELECT DB_NAME(mf.database_id) AS [DBName],
	...
	vfs.num_of_bytes_read + vfs.num_of_bytes_written AS [TotalBytes], --Total number of bytes reads and writes the file.
...
WHERE DB_NAME(mf.database_id) IN ('AutoDealershipDemo','CookbookDemo', 'Doghouse')	-- YUN: Filter for demo
ORDER BY
	[DBName], [FileType] DESC	-- YUN: Changed for demo
	--  [AvgLatency_(ms)] DESC
	--  [AvgReadLatency_(ms)]
	--  [AvgWriteLatency_(ms)]
